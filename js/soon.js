function _toConsumableArray(arr) {if (Array.isArray(arr)) {for (var i = 0, arr2 = Array(arr.length); i < arr.length; i++) {arr2[i] = arr[i];}return arr2;} else {return Array.from(arr);}}var canvas = void 0,ctx = void 0,w = void 0,h = void 0,thunder = void 0,text = void 0,particles = void 0,input = void 0;

function Thunder(options) {var _this = this;
    options = options || {};
    ...
    this.update = function (index, array) {
        this.segments.forEach(function (s) {(s.direct += s.change) && Math.random() > 0.96 && (s.change *= -1);});
...
function Particles(options) {
    ...
    this.sparks = [].concat(_toConsumableArray(new Array(this.max))).map(function () {return new Spark(options);});

    this.update = function () {var _this2 = this;
        this.sparks.forEach(function (s, i) {return s.update(i, _this2.sparks);});
    };

    this.render = function (ctx) {
        this.sparks.forEach(function (s) {return s.render(ctx);});
    };
}
...
function update() {
    text.update();
    thunder.forEach(function (l, i) {return l.update(i, thunder);});
...
function render() {
    ...
    text.render(ctx);
    thunder.forEach(function (l) {return l.render(ctx);});
    particles.forEach(function (p) {return p.render(ctx);});
}
